...
Two layouts, one boundary at **768px** — an iPad in portrait. Desktop and tablet
are the same layout: two-column splits, horizontal nav, all the way down. Below
768 is the phone layout. Everything between the named breakpoints is handled by
`clamp()` on the type, the gutter and the section rhythm, so the layout scales
continuously rather than snapping.

Verified at 24 widths from 2560 down to 320: no horizontal overflow anywhere,
the three rule lengths stay a single value each, the section rule stays the
exact midpoint of the other two, and the label→rule→heading rhythm stays
identical across all five sections.
