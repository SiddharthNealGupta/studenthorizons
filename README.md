...
Anchor links are handled in JS on phones, not left to the browser. The drawer
lives inside the sticky masthead, so while it is open it adds its own height to
the document flow. A native anchor jump resolves the target against that OPEN
layout, then the drawer collapses and pulls everything up underneath the scroll
— landing ~315px past the section, every time. The handler swallows the jump,
closes the drawer, waits for the `max-height` transition to end (with a 420ms
fallback in case it never fires), and only then calls `scrollIntoView`.

`scroll-padding-top` is `calc(var(--head) + var(--rule))`. The masthead is
`--head` tall **plus** its bottom rule; without the rule every anchor lands with
the section's first pixel tucked behind it.

`pushState` keeps the URL shareable, and a `popstate` listener reuses the same
landing logic so the back button moves the page instead of silently changing the
hash.

## Touch vs pointer

The cursor stands down on `touchstart`/`touchmove`/`touchend` — but standing
down is not sufficient on its own. A tap makes the browser synthesise
`mousemove`, `mousedown` and `mouseup` a moment later, and two taps in different
places are indistinguishable from a moving mouse, so the synthetic move turned
the cursor straight back on. Mouse activity within 1000ms of a touch is
therefore ignored outright.

It is a window rather than a permanent switch so a hybrid machine — touchscreen
plus trackpad — gets the cursor back the moment the trackpad is genuinely used.
Verified both directions: mouse → tap stands it down, and a real trackpad move
after the lockout brings it back.
...
Every control that fills — on hover or on click — fills with `--ink`: white in
dark theme, black in light. The text on it takes `--ink-inv`, so it is always
the exact inverse of the plate beneath it. Red keeps every border, rule, label,
emphasis word and the carousel numeral.

### Swap timing is derived, not tuned

Text on a filling control has exactly two safe states: base colour while it sits
on the page, inverse colour while it sits on the plate. The only dangerous
instants are the ones where it is half on each — so the swap runs *during* that
straddle and nowhere else.

For an ink band spanning `[a, b]` of the control's height, the plate's top edge
sits at `H(1.01 − progress)` coming in and `H(progress)` going out:
...
why entering and leaving carry separate values rather than one symmetric delay.
The socials tile is the clearest case: its icon sits high and its label sits
low, so the plate reaches them in opposite order depending on direction. They
are coloured on the children, with near-mirrored timings.

Each progress value goes through the inverse of `--swipe`
(`cubic-bezier(0.65, 0, 0.35, 1)`), multiplied by the fill duration. Bands were
measured in the browser. `--ct` is when the swap starts, `--cd` how long it
runs; both resolve from the state being transitioned *to*, so the base rule
governs leaving and `:hover` governs entering.

| Control | enter `--ct` / `--cd` | leave `--ct` / `--cd` |
|---|---|---|
| `.btn` (all variants) | 0.172 / 0.081 | 0.169 / 0.082 |
| `.socials a svg` | 0.205 / 0.053 | 0.145 / 0.052 |
| `.socials a span` | 0.147 / 0.047 | 0.208 / 0.047 |
| `.car-nav` | 0.137 / 0.080 | 0.135 / 0.082 |
| `.tb-line` | 0.100 / 0.052 | 0.099 / 0.053 |
| `.menu-bars` | 0.124 / 0.012 | 0.123 / 0.012 |

The theme and menu buttons were converted from a cross-fade to a sweep to make
this possible at all: a cross-fade drives plate and ink through the same
mid-grey simultaneously, so there is no instant to swap at.

`.flip-hint` is exempt — it is Horizon Red in every state. It reads on the page
ground and on either ink plate, so it never swaps, which also removes the lag
that made it the one element trailing behind a theme change.

### Verifying it

`transition-delay` is not something to eyeball. The check traces computed colour
against the plate's live `translateY` on every animation frame, and asserts two
invariants: never inverse-coloured while fully on the page, never base-coloured
while fully on the plate. Eight controls × two directions × two themes, all
clean — and the same trace with the delays forced to `0s` fails all thirty-two,
which is what makes the passing result mean anything.

Pixel-sampling was tried first and is useless here: once any black and any white
coexist in the frame, the max-contrast pair reads 21:1 regardless of what the
glyph is doing.

### Do not put `.reveal` on an interactive element

The reveal rules set `transition` wholesale, which silently overrides that
element's own transitions. The CTA button carried `.reveal` and lost its ink
timing entirely. Wrap the control in a `.reveal` span instead; `.rise` keeps the
wrapper out of the layout.

## Cursor

Horizon Red in every state. It used to flip to black over red-filled controls,
because red-on-red vanished; now that fills are ink, the cursor only ever sits
on the page ground (3.4:1) or on an ink plate (6.2:1), so one state covers
everything.

It is deliberately isolated in its own `<script>` tag. Two earlier versions
"went missing" in the field while testing clean in the lab, for two structural
reasons: it sat at the end of the main script, where any earlier throw killed
it; and it was gated on `(hover: hover) and (pointer: fine)`, which query the
**primary** pointer — on a touchscreen laptop that reports coarse even with a
trackpad attached, so the block was skipped. The gate is now the mouse itself:
the first real mousemove turns it on, and a lone synthetic move from a tap does
not qualify.

## Layout bands

Two layouts, one boundary at **48em**. Desktop and tablet are the same layout —
two-column splits, horizontal nav — all the way down. Below 48em is the phone
layout. Between the named breakpoints, `clamp()` on the type, the gutter and the
section rhythm carries everything, so the layout scales continuously.

### Breakpoints are in em, deliberately

In a media query `em` resolves against the **browser's default font size** — the
setting a person changes when they ask for larger text at the OS or browser
level. At the default 16px these values are exactly the pixel values they
replace. When someone raises it, every breakpoint scales with it, so the phone
layout arrives at a wider viewport instead of the tablet nav being asked to hold
type it no longer can.

Browser *zoom* needs no special handling: zoom changes the CSS pixel width of
the viewport, so px-equivalent boundaries already move with it.

Verified with the browser default font at 16, 18, 20 and 24px across 14
viewports each: no horizontal overflow anywhere. At 24px default the carousel
controls were the last thing to break — the dots and arrows are sized in rem, so
they outgrow a phone column even after the small-screen dot rule. Both rows wrap
now.

### Cache busting

`styles.css` and the icons carry `?v=`. **Bump it on every stylesheet change.**
GitHub Pages serves assets with a short max-age, but a browser already holding
the file will keep serving it from disk across ordinary reloads — which is how a
desktop ends up showing older behaviour than a phone that fetched it fresh.

### Logo proportions

On desktop the wordmark is 268px against a 2.375rem toggle — 7.05:1. On phones
the wordmark takes whatever the row has left over rather than a fixed clamp, so
it holds that same ratio (capped at the desktop maximum) instead of collapsing
to a third of it. It is `flex: 0 1 auto`, so if the row ever runs short the mark
gives way first and the nav never does.

The tablet mark cannot reach 7.05:1 — the row still carries five nav links, Join
and the toggle — so it takes the largest size that leaves the nav uncrowded, on
the same 16vw slope as the band above it so it grows continuously across the
boundary.

In the phone footer the mark is locked to `--foot-cta`, the same token that caps
the Become an Ambassador button, so the two centred elements share an edge.

## Headings

The deepest ink extent across both faces is ~1.09em, so a `line-height` under
~1.10 guarantees a descender/ascender collision and 1.12 leaves a fraction of a
pixel. 1.18 (1.24/1.26 on phones, where headings actually wrap) holds a real gap
at every size the type reaches — verified against canvas `TextMetrics` ink
extents rather than by eye.

`.d-br` and `.m-br` are the desktop and phone line breaks; each is inert in the
other band, and both carry an authored space beside them so switching one off
never joins two words.

## Mobile

Below 700px the deliberate `<br>` breaks are switched off and the per-paragraph
caps released, so the copy sets to the column instead of breaking one line then
running three. Each `<br>` has an authored space before it — collapsed at a line
end on desktop, but it is what keeps the words apart once the break stops
rendering.

The nav drawer animates on `max-height` and is held closed with `inert` rather
than `hidden`, which cannot be transitioned. The page is no longer scroll-locked
while it is open.
