...
the exact inverse of the plate beneath it and always measures 21:1.

Red keeps every border, rule, label, emphasis word and the carousel numeral, so
the accent is undiminished. It simply never becomes a plate that text has to
survive on. This replaced red fills, which forced black text onto Horizon Red at
3.4:1 in dark theme.

The colour swap still has to happen *at* the moment the fill passes under the
text, never before — flip early and the text momentarily matches the page it is
still sitting on and disappears. Each `--ink-in-*` delay is the measured
crossing point of its own fill:
...
same edge it was covered from, so it has to return to base colour while the
plate is still behind it.

Measured worst case through the fill, every control, both themes: 12.0:1.
...
timing entirely — its label flipped colour instantly and dropped into the
background mid-fill. Wrap the control in a `.reveal` span instead; `.rise`
